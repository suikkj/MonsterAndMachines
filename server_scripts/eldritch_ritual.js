...
var ER_PHASE1_TICKS = 1200        // Phase 1 ends at 1 minute (consume eldritch_soul_shard)
var ER_PHASE2_TICKS = 2400        // Phase 2 ends at 2 minutes (consume soul_orb_vampire)
...
var ER_CENTER_ITEM = 'irons_spellbooks:eldritch_manuscript'
var ER_OUTER_ITEM = 'discerning_the_eldritch:eldritch_soul_shard'
var ER_INNER_ITEM = 'vampirism:soul_orb_vampire'
var ER_OUTPUT_ITEM = 'blocksyouneed_luna:antimetal_ingot'
var ER_OUTPUT_COUNT = 4

// Pedestal offsets relative to center [dx, dz, requiredItem]
// Cardinals (outer, "mais a ponta") - eldritch_soul_shard
// Diagonals (inner, "mais ao centro") - soul_orb_vampire
// Center - eldritch_manuscript
var ER_PEDESTALS = [
    { dx: 0, dz: 0, item: ER_CENTER_ITEM, phase: 3 },
    // Outer 4 (cardinal directions) - consumed at phase 1
    { dx: 0, dz: -4, item: ER_OUTER_ITEM, phase: 1 },
    { dx: 0, dz: 4, item: ER_OUTER_ITEM, phase: 1 },
    { dx: -4, dz: 0, item: ER_OUTER_ITEM, phase: 1 },
    { dx: 4, dz: 0, item: ER_OUTER_ITEM, phase: 1 },
    // Inner 4 (diagonal positions) - consumed at phase 2
    { dx: -2, dz: -2, item: ER_INNER_ITEM, phase: 2 },
    { dx: 2, dz: -2, item: ER_INNER_ITEM, phase: 2 },
    { dx: -2, dz: 2, item: ER_INNER_ITEM, phase: 2 },
    { dx: 2, dz: 2, item: ER_INNER_ITEM, phase: 2 }
]
...
function erCheckPedestalItems(level, cx, cy, cz) {
    for (var i = 0; i < ER_PEDESTALS.length; i++) {
        var ped = ER_PEDESTALS[i]
        var itemId = erGetPedestalItemId(level, cx + ped.dx, cy, cz + ped.dz)
        if (itemId !== ped.item) {
            console.info('[Eldritch Ritual] FAIL: Pedestal item at offset (' + ped.dx + ',0,' + ped.dz + ') expected ' + ped.item + ' but got ' + itemId)
            return false
        }
    }
...
function erCheckRemainingItems(level, cx, cy, cz, currentPhase) {
    // Check only unconsumed pedestal items
    for (var i = 0; i < ER_PEDESTALS.length; i++) {
        var ped = ER_PEDESTALS[i]
        if (ped.phase > currentPhase) {
            var itemId = erGetPedestalItemId(level, cx + ped.dx, cy, cz + ped.dz)
            if (itemId !== ped.item) return false
        }
    }
    return true
}
...
ServerEvents.tick(function (event) {
    ...
    if (tick % 20 === 0) {
        ...
        for (var k = 0; k < keys.length; k++) {
            ...
            if (!erCheckRemainingItems(level, cx, cy, cz, currentPhase)) {
            ...
            if (elapsed >= ER_PROCESS_TICKS) {
                ...
                // Spawn antimetal at center
                for (var j = 0; j < ER_OUTPUT_COUNT; j++) {
                    server.runCommandSilent('execute in ' + dim + ' positioned ' + cx + ' ' + (cy + 1) + ' ' + cz + ' run summon minecraft:item ~ ~ ~ {Item:{id:"' + ER_OUTPUT_ITEM + '",count:1},PickupDelay:40}')
                }
                // Completion effects
            ...
            if (elapsed % 60 === 0) {
                server.runCommandSilent('execute in ' + dim + ' positioned ' + cx + ' ' + cy + ' ' + cz + ' run immersivemessages sendcustom @a[distance=..32] {anchor:"BOTTOM_CENTER",color:"#AA00AA",size:0.9,y:-30.0,italic:1} 3 \u2726 Ritual Eldritch: ' + pct + '% (' + minutesLeft + ' min) \u2726')
            }
        }
    }
})
...
// ============ ACTIVATION TRIGGER ============
// Place eldritch_manuscript on center pedestal LAST to activate the ritual
BlockEvents.rightClicked('ars_nouveau:arcane_pedestal', function (event) {
    try {
        ...
        // Only trigger when player is holding the center item (manuscript)
        if (!event.item || event.item.id !== ER_CENTER_ITEM) return
        ...
        server.scheduleInTicks(2, function () {
            try {
                ...
                // Verify the manuscript actually made it onto the pedestal
                if (erGetPedestalItemId(capturedLevel, bx, by, bz) !== ER_CENTER_ITEM) return
                ...
                if (!erCheckPedestalItems(capturedLevel, bx, by, bz)) return
                ...
                capturedServer.runCommandSilent('execute in ' + dim + ' positioned ' + bx + ' ' + by + ' ' + bz + ' run immersivemessages sendcustom @a[distance=..32] {anchor:"CENTER_CENTER",color:"#5500AA",size:1.6,bold:1,shake:1,typewriter:1,sound:1,slideup:1,slideoutdown:1,background:1,borderTop:"#AA00AA",borderBottom:"#220044"} 6 \u2726 Ritual Eldritch Iniciado \u2726')
                capturedServer.runCommandSilent('execute in ' + dim + ' positioned ' + bx + ' ' + by + ' ' + bz + ' run immersivemessages sendcustom @a[distance=..32] {anchor:"CENTER_CENTER",color:"#777777",size:1.0,italic:1,typewriter:1,sound:1,y:18.0,slideup:1,slideoutdown:1} 6 O veu entre os mundos comeca a rasgar...')
            } catch (e) {
...
console.info('[Eldritch Ritual] Loaded - Place eldritch_manuscript on center pedestal to activate')
