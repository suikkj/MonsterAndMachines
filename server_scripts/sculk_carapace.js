...
var CARAPACE_CHANCE = 0.02            // 2% chance per qualifying block per check

// What sculk block becomes carapace
var CARAPACE_BLOCK = 'minecraft:reinforced_deepslate'  // Using reinforced_deepslate as carapace
...
function countSculkNeighbors(level, x, y, z) {
    ...
    for (var i = 0; i < offsets.length; i++) {
        ...
        try {
            ...
            if (neighbor && SCULK_NEIGHBORS[neighbor.id]) {
                count++
            }
        } catch (e) { }
    }

    // Also count carapace blocks as neighbors (they help formation)
    for (var j = 0; j < offsets.length; j++) {
        var o2 = offsets[j]
        try {
            var neighbor2 = level.getBlock(x + o2[0], y + o2[1], z + o2[2])
            if (neighbor2 && neighbor2.id === CARAPACE_BLOCK) {
                count++
            }
        } catch (e) { }
    }
