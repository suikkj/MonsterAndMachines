...
// ============ CONFIGURATION ============
var MOB_SCAN_INTERVAL = 200          // Check every 10 seconds (200 ticks)
...
function countSculkBlocks(level, centerX, centerY, centerZ, radius) {
    ...
    for (var s = 0; s < samples; s++) {
        var rx = Math.floor((Math.random() * radius * 2) - radius)
        var ry = Math.floor((Math.random() * 6) - 3) // ±3 vertical
        var rz = Math.floor((Math.random() * radius * 2) - radius)
