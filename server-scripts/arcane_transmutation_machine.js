...
var ATM_RECIPES = [
    ...
    },
    {
        ...
        inputs: [
            { id: 'irons_spellbooks:bloody_vellum', count: 2 },
            { id: 'irons_spellbooks:ender_rune', count: 1 },
            { id: 'irons_spellbooks:cinderous_soulcaller', count: 1 }
        ],
        outputs: [
            { id: 'irons_spellbooks:dragonskin', count: 8 }
        ]
    },
    {
        name: 'Fragmento de Manuscrito',
        inputs: [
            { id: 'minecraft:echo_shard', count: 2 },
            { id: 'minecraft:paper', count: 1 },
            { id: 'irons_spellbooks:timeless_slurry', count: 1 }
        ],
        outputs: [
            { id: 'irons_spellbooks:ancient_knowledge_fragment', count: 4 }
        ]
    }
]
            ...
            for (var dy = -r; dy <= r; dy++) {
                for (var dz = -r; dz <= r; dz++) {
                    try {
                        ...
                        server.runCommandSilent('execute in ' + dim + ' positioned ' + bx + ' ' + by + ' ' + bz + ' run immersivemessages sendcustom @a[distance=..32] {anchor:\"CENTER_CENTER\",color:\"#3295d6ff\",size:1.5,bold:1,obfuscate:1,typewriter:1,sound:1,slideup:1,slideoutdown:1,borderTop:\"#FF55FF\",borderBottom:\"#5500AA\"} 10 Transmutacao Arcana Iniciada ')
                        server.runCommandSilent('execute in ' + dim + ' positioned ' + bx + ' ' + by + ' ' + bz + ' run immersivemessages sendcustom @a[distance=..32] {anchor:\"CENTER_CENTER\",color:\"#a43aceff\",size:1.0,italic:1,typewriter:1,sound:1,y:15.0,slideup:1,slideoutdown:1} 5 Transmutando: ' + result.recipe.name + '')
