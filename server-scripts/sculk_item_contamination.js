    ...
    for (var p = 0; p < players.size(); p++) {
        ...
        try {
            ...
            for (var e = 0; e < entities.size(); e++) {
                ...
                // Each item in the stack rolls independently
                // Staaaaaaaaaaaack mod compatibility: reads real count from entity NBT
                try {
                    var itemStack = entity.item
                    var transformation = SCULK_TRANSFORMATIONS[itemStack.id]
                    if (transformation && timeOnSculk >= DESTROY_THRESHOLD_TICKS) {
                        // Get the REAL item count (Staaaaaaaaaaaack stores count beyond 64)
                        // Try reading from entity NBT first, fallback to itemStack.count
                        var itemCount = itemStack.count
                        try {
                            var entityNbt = entity.nbt
                            if (entityNbt) {
                                // Staaaaaaaaaaaack stores extra count in the Item compound
                                var itemNbt = entityNbt.get('Item')
                                if (itemNbt) {
                                    var nbtCount = itemNbt.getInt('count')
                                    if (!nbtCount) nbtCount = itemNbt.getInt('Count')
                                    if (nbtCount && nbtCount > itemCount) {
                                        itemCount = nbtCount
                                    }
                                }
                                // Also check for staaaaaaaaaaaack-specific NBT field
                                var stxckCount = entityNbt.getInt('staaaaaaaaaaaack:count')
                                if (!stxckCount) stxckCount = entityNbt.getInt('StxckCount')
                                if (stxckCount && stxckCount > itemCount) {
                                    itemCount = stxckCount
                                }
                            }
                        } catch (nbtEx) {
                            // NBT reading failed, use itemStack.count
                        }

                        // Safety: ensure at least 1
                        if (itemCount < 1) itemCount = 1

                        // DEBUG: Log to verify Staaaaaack count detection (remove after confirming fix)
                        console.info('[Sculk Transform] Item: ' + itemStack.id + ' | itemStack.count=' + itemStack.count + ' | realCount=' + itemCount)
                        ...
                        if (successCount > 0) {
                            // Spawn transformed items in batches of 64 to avoid NBT byte overflow
                            var remaining = successCount
                            while (remaining > 0) {
                                var batchSize = Math.min(remaining, 64)
                                server.runCommandSilent('execute in ' + dimKey + ' positioned ' + itemPos.x + ' ' + itemPos.y + ' ' + itemPos.z + ' run summon minecraft:item ~ ~0.5 ~ {Item:{id:"' + transformation.output + '",count:' + batchSize + '},PickupDelay:20}')
                                remaining -= batchSize
                            }
