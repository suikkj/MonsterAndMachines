...
PlayerEvents.inventoryChanged(function (event) {
    ...
    if (FULL_RESTRICTED[itemId]) {
        event.item.count = 0
        event.server.runCommandSilent('title ' + username + ' actionbar {"text":"Apenas Arquivista e Kali podem possuir este item!","color":"red","bold":true}')
    ...
    if (CRAFT_ONLY_RESTRICTED[itemId]) {
        ...
        event.item.count = 0
        event.server.runCommandSilent('title ' + username + ' actionbar {"text":"Apenas Arquivista e Kali podem fabricar este item!","color":"red","bold":true}')
        event.server.runCommandSilent('playsound minecraft:block.note_block.bass master ' + username + ' ~ ~ ~ 1 0.5')
    }
})
