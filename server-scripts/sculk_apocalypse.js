...
PlayerEvents.tick(function (event) {
    ...
    if (onSculk) {
        ...
        if (hasProtectiveBoots) {
            ...
            // Clear debuffs via API (PERFORMANCE: replaces runCommandSilent)
            for (var i = 0; i < SCULK_DEBUFFS_AP.length; i++) {
                player.removeEffect(SCULK_DEBUFFS_AP[i])
            }
            ...
            if (!hasAnyDebuff) {
                // Apply random debuff via API (PERFORMANCE: replaces runCommandSilent)
                var randomIdx = Math.floor(Math.random() * SCULK_DEBUFFS_AP.length)
                var effectId = SCULK_DEBUFFS_AP[randomIdx]
                player.potionEffects.add(effectId, 200, 0, false, false)
            }
        ...
        if (lastOffTime === 0) {
            ...
            if (ticksOffSculk > OFF_SCULK_GRACE_PERIOD_AP) {
                ...
                for (var k = 0; k < SCULK_DEBUFFS_AP.length; k++) {
                    player.removeEffect(SCULK_DEBUFFS_AP[k])
                }
            }
        }
    }
